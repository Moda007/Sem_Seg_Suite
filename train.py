...
for epoch in range(args.epoch_start_i, args.num_epochs):
    ...
    if not os.path.isdir("%s%s/%04d"%(main_dir, "checkpoints", epoch)):
        os.makedirs("%s%s/%04d"%(main_dir, "checkpoints", epoch))
    ...
    if val_indices != 0 and epoch % args.checkpoint_step == 0:
        ...
        saver.save(sess,"%s%s/%04d/model.ckpt"%(main_dir, "checkpoints", epoch))


    if epoch % args.validation_step == 0:
        print("Performing validation")
        target=open("%s%s/%04d/val_scores.csv"%(main_dir, "checkpoints", epoch),'w')
        ...
        for ind in val_indices:
            ...
            cv2.imwrite("%s%s/%04d/%s_pred.png"%(main_dir, "checkpoints", epoch, file_name),cv2.cvtColor(np.uint8(out_vis_image), cv2.COLOR_RGB2BGR))
            cv2.imwrite("%s%s/%04d/%s_gt.png"%(main_dir, "checkpoints", epoch, file_name),cv2.cvtColor(np.uint8(gt), cv2.COLOR_RGB2BGR))
